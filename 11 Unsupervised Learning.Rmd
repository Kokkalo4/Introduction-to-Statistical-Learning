...
apply(USArrests,2,var) #Principal components is about variance and so mean shouldnt play a role, but the variance of individual variables will play a role.
```
...
```{r}
km.out <- kmeans(x,4, nstart = 15) #we tell kmeans that there are 4 clusters and we tell it to do 15 random starts.
km.out
...
We will use the same data and use hierarchical clustering.
