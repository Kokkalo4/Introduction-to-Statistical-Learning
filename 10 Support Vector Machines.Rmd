...
To demonstrate the SVM, it is easiest to work on low dimensions, so we can see the data. 
...
```{r}
set.seed(10011)
x <- matrix(rnorm(40),20,2) #make a matrix x that has 40 numbers in 20 rows and 2 columns.
y <- rep(c(-1,1),c(10,10)) 
x[y = 1,] = x[y = 1,] + 1
...
The first thing we will do is make a grid for values X1 and X2. We will write a function to do that in case we want to reuse it. It uses the handy function `expand.grid` and produces the coordinates of `n*n` points on a lattice covering the domain of `x`. Having made the lattice, we make a prediction at each point on the lattice. We then plot the lattice, color coded according to the classification. Now we can see the decision boundary. 
